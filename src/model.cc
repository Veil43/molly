...
MaterialHandle load_material_to_opengl(MaterialData& m, bool flip_textures) {
    MaterialHandle output_material = {};
    
    output_material.diff_image = molly::load_image_file(m.diffuse.c_str(), flip_textures);
    output_material.norm_image = molly::load_image_file(m.normal.c_str(), flip_textures);
    output_material.spec_image = molly::load_image_file(m.specular.c_str(), flip_textures);
...
static MaterialData parse_material(const std::string& dir, const std::string& file) {
    ...
    while(std::getline(file_contents, line)) {
        ...
        if (prefix == "map_kd" && !diff_found) {
            ...
            std::string file;
            iss >> file;
            output_material.specular = dir + file;
            spec_found = true;
        }
