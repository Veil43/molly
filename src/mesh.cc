...
StaticMesh::StaticMesh(Vertex* vertex_data, u32* index_data, i32 vcount, i32 icount)
    : m_vertex_count{vcount}, m_index_count{icount}
{
    GL_QUERY_ERROR(glGenVertexArrays(1, &m_vao);)
    GL_QUERY_ERROR(glBindVertexArray(m_vao);)
    GL_QUERY_ERROR(glGenBuffers(1, &m_vbo);)
    GL_QUERY_ERROR(glBindBuffer(GL_ARRAY_BUFFER, m_vbo);)
    GL_QUERY_ERROR(glBufferData(GL_ARRAY_BUFFER, m_vertex_count * sizeof(Vertex), (void*)vertex_data, GL_STATIC_DRAW);)
    if (index_data != nullptr) {
        GL_QUERY_ERROR(glGenBuffers(1, &m_ebo);)
        GL_QUERY_ERROR(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ebo);)
        GL_QUERY_ERROR(glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_index_count * sizeof(u32), (void*)index_data, GL_STATIC_DRAW);)
    } else {
        m_index_count = 0;
    }
